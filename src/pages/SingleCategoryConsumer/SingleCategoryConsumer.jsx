...
import "./SingleCategoryConsumer.scss";
...
const CategoryCard = lazy(() => import("./Components/FilterCategoryCard/FilterCategoryCard"));
...
const SingleCategoryConsumer = () => {
  ...
  const { data: SeoData } = useSEO(`${REACT_APP_API_URL}page/single/?url=listing`);
  ...
  const getAllBusiness = async () => {
    ...
    const params = {
      ...
      ...(verified && { verified_status: "ACTIVE" }),
  ...
  return (
    <Suspense fallback={<Loader />}>
      <main className="single__category__page">
        ...
        <div className="singlecate__top">
          <div className="container text-center">
            <div className="d-flex justify-content-center breadcrumb__wrap">
              <Breadcrumb extra="Categories" active={formattedName && formattedName} />
            </div>
            {formattedName && <h1 className="text-center text-capitalize">Best in {formattedName}</h1>}
              ...
              <div className="col-lg-8">
                <div className="filter__sorting__area">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <div className="sort__by">
                      <span>Sort by</span>
                      <select className="form-select py-3" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                ...
                <div className="all__filter__results">
                  {loading ? (
                    ...
                    data.map((item, key) => <CategoryCard data={item} key={key} />)
                  ) : (
                    <>
                      <NoDataFound />
        ...
        <PopularSearch />
        <section className="recently__reviews__companies fill__sec">
          <div className="container">
            <h2 className="text-center">Recently reviewed companies</h2>
            <div className="rec__comp__box row">{reviews && reviews.map((review, key) => <ReviewMainBox key={key} data={review} />)}</div>
          </div>
        </section>
      </main>
    </Suspense>
