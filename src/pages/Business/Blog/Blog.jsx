...
const Blog = () => {
  ...
  return (
    <Suspense fallback={<Loader />}>
      <SEO title={data?.meta_title || "TrustyFeed"} description={data?.meta_description} keywords="TrustyFeed" />
      <div className="our__blog__page">
        ...
        <section className="floating__blogs ">
          <div className="container">
            ...
            <div className="f__blogs__wrap clearfix">{loading ? <BlogLoader /> : featureData && featureData.length > 0 ? featureData.slice(0, 4).map((item, key) => <FloatingBlog data={item} key={key} />) : <NoDataFound />}</div>
          </div>
        </section>
